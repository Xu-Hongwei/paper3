...
def summarize(rows):
    ...
    new_cross = len(new_cross_rows)

    new_cross_rank_better = sum(
        row["rank_improvement"] > 0
        for row in new_cross_rows
    )
    new_cross_rank_same = sum(
        row["rank_improvement"] == 0
        for row in new_cross_rows
    )
    new_cross_rank_worse = sum(
        row["rank_improvement"] < 0
        for row in new_cross_rows
    )

    # 对“新跨类错误”再细分：
    # - benign flip: 类别 Top1 变差，但 GT instance rank 变好；
    # - harmful regression: 类别 Top1 变差且 GT rank 同时变差。
    benign_new_cross = new_cross_rank_better
    harmful_new_cross = new_cross_rank_worse
    ...
    return {
        ...
        "target_cross_error_flow": {
            ...
            "new_cross_rank_better": new_cross_rank_better,
            "new_cross_rank_same": new_cross_rank_same,
            "new_cross_rank_worse": new_cross_rank_worse,
            "new_cross_rank_better_rate": (
                new_cross_rank_better / max(new_cross, 1)
            ),
            "new_cross_rank_same_rate": (
                new_cross_rank_same / max(new_cross, 1)
            ),
            "new_cross_rank_worse_rate": (
                new_cross_rank_worse / max(new_cross, 1)
            ),
            "benign_new_cross": benign_new_cross,
            "harmful_new_cross": harmful_new_cross,
        },
        "category_rank_2x2": {
            # 类别状态改善：baseline cross -> reliable non-cross
            "category_better_rank_better": sum(
                row["baseline_type"] == "cross_category_error"
                and row["reliable_type"] != "cross_category_error"
                and row["rank_improvement"] > 0
                for row in rows
            ),
            "category_better_rank_same": sum(
                row["baseline_type"] == "cross_category_error"
                and row["reliable_type"] != "cross_category_error"
                and row["rank_improvement"] == 0
                for row in rows
            ),
            "category_better_rank_worse": sum(
                row["baseline_type"] == "cross_category_error"
                and row["reliable_type"] != "cross_category_error"
                and row["rank_improvement"] < 0
                for row in rows
            ),
            # 类别状态恶化：baseline non-cross -> reliable cross
            "category_worse_rank_better": new_cross_rank_better,
            "category_worse_rank_same": new_cross_rank_same,
            "category_worse_rank_worse": new_cross_rank_worse,
        },
        "rank_change_all": {
            "better": rank_better,
            "same": rank_same,
...
def print_summary(summary):
    ...
    print(
        f"  ├─ Rank better     : "
        f"{flow['new_cross_rank_better']} "
        f"({flow['new_cross_rank_better_rate']:.2%})"
    )
    print(
        f"  ├─ Rank same       : "
        f"{flow['new_cross_rank_same']} "
        f"({flow['new_cross_rank_same_rate']:.2%})"
    )
    print(
        f"  └─ Rank worse      : "
        f"{flow['new_cross_rank_worse']} "
        f"({flow['new_cross_rank_worse_rate']:.2%})"
    )
    print(
        f"Benign new-cross     : {flow['benign_new_cross']} "
        "(类别Top1变差，但GT实例rank改善)"
    )
    print(
        f"Harmful regression   : {flow['harmful_new_cross']} "
        "(类别Top1变差，GT实例rank也恶化)"
    )

    matrix = summary["category_rank_2x2"]
    print("\n[3] Category transition × Instance-rank change")
    print(
        "Category better | "
        f"Rank better/same/worse = "
        f"{matrix['category_better_rank_better']}/"
        f"{matrix['category_better_rank_same']}/"
        f"{matrix['category_better_rank_worse']}"
    )
    print(
        "Category worse  | "
        f"Rank better/same/worse = "
        f"{matrix['category_worse_rank_better']}/"
        f"{matrix['category_worse_rank_same']}/"
        f"{matrix['category_worse_rank_worse']}"
    )

    print("\n[4] Baseline cross-error A/B/C/D")
    ...
    print("2) A 区是否贡献主要 correction；")
    print("3) New cross 中多少其实是 RankBetter 的 benign flip；")
    print("4) 真正 harmful regression = new cross 且 RankWorse；")
    print("5) 全部 caption 的 GT rank 是否整体向前移动。")
    print("=" * 96)
...
def main():
    ...
    html_specs = [
        ...
        (
            "new_cross_rank_better",
            "Benign New Cross Errors: Category Worse but GT Rank Better",
            lambda row: (
                row["baseline_type"]
        ...
        (
            "new_cross_rank_worse",
            "Harmful New Cross Errors: Category Worse and GT Rank Worse",
            lambda row: (
                row["baseline_type"]
                != "cross_category_error"
                and row["reliable_type"]
